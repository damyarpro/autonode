...
| بوم‌های من | `#/boards` | ساخت بوم، تغییر نام، خصوصی/عمومی، حذف |
| ویرایشگر بوم | `#/boards/:slug` | راست‌کلیک برای پالت نود، وصل‌کردن، گروه، undo، ذخیره، هیستوری |
...
| ورودی هر پنج کانال | واقعی — وبهوک امضاشده، و برای تلگرام مسیر مخفی |
| خروجی وب‌سایت | واقعی با `WEBSITE_PUBLISH_URL` — یک POST امضاشده به اندپوینت خودت؛ هیچ تأییدی لازم ندارد |
| خروجی لینکدین | کد واقعی با `LINKEDIN_ACCESS_TOKEN`؛ دسترسی API تأییدشده می‌خواهد — **از اینجا هرگز اجرا نشده** |
| خروجی اینستاگرام | کد واقعی با `INSTAGRAM_ACCESS_TOKEN`؛ App Review می‌خواهد، و متنِ بدون عکس یا ویدیو را با `instagram:needs_media` رد می‌کند — **اجرا نشده** |
| خروجی یوتیوب | کد واقعی با refresh token؛ ممیزی گوگل می‌خواهد، و متن بدون فایل ویدیو را با `youtube:needs_video` رد می‌کند — **اجرا نشده** |
| مقصد انتشار هر کانال | واقعی، روی پروفایل بیزینسی؛ کانالِ بدون مقصد نمی‌تواند منتشر کند |
| بوم‌هایی که خودت می‌سازی | واقعی، روی `#/boards` — راست‌کلیک، پالت ۷۷ نودی، وصل‌کردن، گروه، undo، ذخیره |
| هیستوری بوم | واقعی — هر ذخیره یک نسخه است و بازگرداندن، نسخه‌ی تازه می‌سازد نه اینکه چیزی را پاک کند |
| بوم عمومی | واقعی — بدون نشست خوانده می‌شود؛ بوم خصوصی دقیقاً مثل بوم ناموجود جواب می‌دهد |
| نتیجه‌ی تماس | واقعی — `POST /api/webhooks/vapi/:secret` مدت و دلیل پایان و خلاصه را ثبت می‌کند — **اجرا نشده** |
...
| وبهوک پرداخت | بسته شد — فقط تأییدیه‌ای که همین سرور امضا کرده پذیرفته می‌شود |
| خروج از حساب | واقعی، وقتی رمز تنظیم شده باشد — بدون رمز اصلاً نشستی نیست که از آن خارج شوی |
| دو دوره، مدیریت اشتراک، تنظیمات حریم خصوصی | ساخته نشده — به‌جای تظاهر، بج **به‌زودی** دارند |
